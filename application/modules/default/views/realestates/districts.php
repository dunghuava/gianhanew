	...
	<div class="container">
		<div class="row">
			...
			<div class="col-xs-12 col-sm-8 col-md-8 nopadding-rgt">
				...
				<div class="block-big">
					<div class="col-xs-12">
						<h1 class="title bar margintTop"><span><?= $meta_title; ?></span></h1>
					</div>
							...
							<article class="col-xs-6 col-md-6 product mb5 p5">
								<div class="item">
									...
									<div class="col-xs-8 col-md-8 description">
										<h2><a class="<?= $realestate->type_class;?>" href="<?=$domain.$realestate->slug_cate.'/'.$realestate->title_alias.'-'.$realestate->id;?>" title="<?= $realestate->title; ?>"><?= stripString($realestate->title,65,'.'); ?></a></h2>
										<div class="cusInfo">
										...
										<div class="cusInfo location">
											<span>
												<a href="<?= $domain.$realestate->slug_cate.'/'.$realestate->slug_p.'-tp'.toPublicId($realestate->province_id); ?>.htm" title="<?= $realestate->title_cate.' '.$realestate->p_name; ?>"><?= $realestate->p_name; ?></a>
											</span>
											<span class="hidden-xs hidden-sm date pull-right">
                                            <?php
                                                if (strtotime(date('d-m-Y',strtotime($realestate->updated_at))) == strtotime(date('m/d/Y'))){
                                                    echo '<span class="hidden-xs hidden-sm date today pull-right">Hôm nay</span>';
                                                }else{
									...
									foreach ($districts as $district) {
                                       ...
                                       if($countRowTable > 0){
                                          ...
                                          <li>
											<i class="fa fa-caret-right"></i>
											<h4>
												<a href='<?= $domain.$category->title_alias.'/'. $province->slug_name.'/'.$district->slug_name;?>.htm' title='<?= $category->title.' tại'. $district->pre. ' ' .$district->name; ?>'>
                                                <?= $district->name . '<span class="countR">('.$countRowTable.')</span>'; ?>
                                                </a>
											</h4>
										 </li>
                                          <?php
