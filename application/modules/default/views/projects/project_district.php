		...
		<div class="row">
			...
			<div class="col-xs-12 col-sm-8 col-md-8 nopadding-rgt">
				<div class="block-big">
								...
								<form action="<?= $domain.'tim-kiem-du-an.htm'; ?>" method="POST" class="form-inline" role="form">
									...
									<div class="form-group col-md-3 nopadding-rgt">
										<select class="form-control" name="sltProjectProvince" id="sltProjectProvince">
											...
											?>
											<option value="<?= $province->province_id ?>" <?= ($province->province_id == $district->province_id) ? 'selected' : ''; ?>><?= $province->name ?></option>
											<?php
												}
											}
											?>
									...
									<div class="form-group col-md-3">
										<select class="form-control" name="sltProjectDistrict" id="sltProjectDistrict">
											...
											?>
											<option value="<?= $sltD->district_id; ?>" <?= $sltD->district_id == $district->district_id ? 'selected' : ''; ?>><?= $sltD->name;?></option>
											<?php
												}
											}
											?>
					...
					<div class="col-xs-12">
						<h1 class="title bar"><span>Dự án <?= $category->title . ' tại ' . $district->name . ' ' .$district->province_name; ?></span></h1>
					</div>
					<!-- Data -->
					<div class="total">
						<?php
						...
						<div class="col-sm-6 col-md-6">
							<div class="project">
								<div class="post-img">
									<a href="<?= $domain.$project->title_alias.'-pj'.$project->id; ?>" title="<?= $project->title ?>">
								...
								<h3 class="text-center">
									<a class="post-title" href="<?= $domain.$project->title_alias.'-pj'.$project->id; ?>" title="<?= $project->title ?>" title="<?= $project->title ?>"><?= $project->title ?></a>
								</h3>
							</div>
						</div>
						<?php
